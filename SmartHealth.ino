#include <C:\Users\KEERTHIVASAN\AppData\Local\Arduino15\packages\esp32\hardware\esp32\1.0.6\libraries\WiFi\src\WiFi.h>
#include <WebServer.h>
#include <Wire.h>
...
char auth[] = "UfVwwD8sC5kX_H8JPSSEyKBsD_x1lH3R"; 

const char* host = "api.thingspeak.com";
const char* api_key= "QGNW8F7ZVXA145PU";
unsigned long chanNum = 1698963;
 ...
 void SEND_LCD()
{
  ...
  lcd1.clear();
  lcd1.print(0, 0, temperature_text);
  lcd1.print(0, 1, humidity_text); 

  Blynk.virtualWrite(V2, bodytemperature);
  Blynk.virtualWrite(V3, BPM);
  Blynk.virtualWrite(V4, SpO2);
  if (bodytemperature>37)
  {
    Blynk.notify("The patient is suffering from fever. Kindly give attention!!");
  }
}
...
void setup() {
  ...
  timer.setInterval(1000L, SEND_LCD);
...
void loop() {
  ...
  if (millis() - tsLastReport > REPORTING_PERIOD_MS) 
  {
    ...
    ThingSpeak.writeFields(chanNum, api_key);
    delay(100);
...
void handle_NotFound(){
  ...
  ptr +=".icon{width:65px}";
  ptr +=".keerthi{display: block;width: 100%; border: none;background: #FF0000; color: white; padding: 14px 28px; font-size: 20px; cursor: pointer; text-align: center;}";
  ptr +="</style>";
  ptr +="</head>";
  ptr +="<body>";
  ptr +="<h1>ESP32 based Smart Health Monitoring</h1>";
  ptr +="<h3>IoT Domain Analyst Project</h3>";
  ...
  ptr +="<span class='superscript'>&deg;C</span></div>";
  ptr +="<a class='keerthi' href=\"https://thingspeak.com/channels/1698963/private_show\" target=\"_blank\">THINGSPEAK CLOUD</a>";
  ptr +="</div>";
  ptr +="</div>";
  ptr +="</body>";
  ptr +="</html>";
